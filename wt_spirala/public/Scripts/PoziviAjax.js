...
/*
        ...
        if (ajax.readyState === 4) {
            if (ajax.status === 200) {
                try {
                    ...
                    fnCallback('JSON parse error', null);
                }
            } else {
                try {
                    const errorResponse = JSON.parse(ajax.responseText);
                    fnCallback(errorResponse.greska || ajax.statusText, null);
                } catch (e) {
                    fnCallback('Unknown error', null);
                }
            }
        }
    };
    ...
    // vraća top 5 nekretnina za određenu lokaciju
    ...
    // vraća sve upite za trenutno loginovanog korisnika
    ...
    // vraća detalje određene nekretnine
    ...
    // vraća sljedeće upite za nekretninu na osnovu stranice
